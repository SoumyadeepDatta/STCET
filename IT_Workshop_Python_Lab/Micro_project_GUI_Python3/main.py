...
Label(root, text="ST. THOMAS' COLLEGE OF ENGINEERING AND TECHNOLOGY",
      font='Britanic 18 bold').place(x=70, y=30)
Label(root, text='ADDRESS - 4, Diamond Harbour Road, Kidderore, Kolkata-7000023',
      font='Arial 12 italic').place(x=155, y=538)
Label(root, text='PHONE - 2448-1081/1082',
      font='Arial 12 italic').place(x=300, y=580)
Label(root, text='EMAIL-ID - www.stcet.org',
      font='Arial 12 italic').place(x=310, y=620)
...
photo = Label(root, text="Photo to be inserted here.",
              font="Arial 12").place(x=780, y=220)
...
def page1():
    ...
    def page2():
        ...
        Label(win2, text="Educational Details", font='Georgia 23',
              relief=GROOVE).grid(row=0, column=0, padx=50, pady=20)
        Label(win2, text="Department", font='Arial 12 bold').grid(
            row=3, column=0, padx=20, pady=20)
        Label(win2, text='Class 10', font='Arial 12 bold').grid(
            row=4, column=0, padx=20, pady=20)
        Label(win2, text='Percentage', font='Arial 12 bold').grid(
            row=4, column=2, padx=20, pady=20)
        Entry(win2, textvariable=class10_var).grid(
            row=4, column=3, padx=20, pady=20)
        Label(win2, text='%').grid(row=4, column=4, padx=20, pady=20)
        Label(win2, text='Class 12', font='Arial 12 bold').grid(
            row=5, column=0, padx=20, pady=20)
        Label(win2, text='Percentage', font='Arial 12 bold').grid(
            row=5, column=2, padx=20, pady=20)
        Entry(win2, textvariable=class12_var).grid(
            row=5, column=3, padx=20, pady=20)
        Label(win2, text='%').grid(row=5, column=4, padx=20, pady=20)
        Label(win2, text='Class 12 PCM Marks', font='Georgia 23',
              relief=GROOVE).grid(row=6, column=0, padx=50, pady=20)
        Label(win2, text='Physics', font='Arial 12 bold').grid(
            row=7, column=0, padx=20, pady=20)
        Label(win2, text='Chemistry', font='Arial 12 bold').grid(
            row=8, column=0, padx=20, pady=20)
        Label(win2, text='Mathematics', font='Arial 12 bold').grid(
            row=9, column=0, padx=20, pady=20)
        Label(win2, text='PCM percentage', font='Arial 12 bold').grid(
            row=10, column=0, padx=20, pady=20)
        Label(win2, text='%').grid(row=10, column=2, padx=20, pady=20)
        Entry(win2, textvariable=Phy_var).grid(
            row=7, column=1, padx=20, pady=20)
        Entry(win2, textvariable=Chem_var).grid(
            row=8, column=1, padx=20, pady=20)
        Entry(win2, textvariable=Maths_var).grid(
            row=9, column=1, padx=20, pady=20)
        Entry(win2, textvariable=pcm_avg).grid(
            row=10, column=1, padx=20, pady=20)
        ...
        def resetPage2():
            result = messagebox.askquestion(
                "CLEAR", "Are you sure you want to reset?")
            if(result == 'yes'):
                class10_var.set("")
                class12_var.set("")
                Phy_var.set("")
        ...
        def store():
            ...
            if(firstname == "" or lastname == "" or gender_c == 0 or fathername == "" or mothername == "" or DOB1 == "" or DOB2 == "" or DOB3 == "" or phno == "" or fphno == "" or email_id_entry == "" or eas == 'Domain_name' or curr_add == "" or per_add == "" or dept == 'DEPT' or board_10 == 'BOARD' or board_12 == 'BOARD' or class_10_per == "" or class_12_per == "" or Phy_marks == "" or Chem_marks == "" or Maths_marks == "" or pcm_per == 0.0):
                ...
                print('Error!')
                messagebox.showerror(
                    "Error :(", "The entered phone number is incorrect!")
                f_name.set("")
                l_name.set("")
                x.set(0)
                father_name.set("")
                ...
                print('Error!')
                messagebox.showerror(
                    "Error :(", "The entered date of birth format is incorrect!")
                f_name.set("")
                l_name.set("")
                x.set(0)
                father_name.set("")
                ...
                result = messagebox.askquestion(
                    "SUBMIT", "Are you sure you want to enter the data?")
                if(result == 'yes'):
                    print('Done')
                    ...
                    if (gender_c == 1):
                        ...
                        sheet.cell(row=current_row + 1,
                                   column=3).value = "Female"
                    else:
                        sheet.cell(row=current_row + 1,
                                   column=3).value = "Other"
                    sheet.cell(row=current_row + 1,
                               column=4).value = fathername
                    sheet.cell(row=current_row + 1,
                               column=5).value = mothername
                    sheet.cell(row=current_row + 1, column=6).value = DOB
                    sheet.cell(row=current_row + 1,
                               column=7).value = phone_number
                    sheet.cell(row=current_row + 1,
                               column=8).value = father_phone_no
                    ...
                    sheet.cell(row=current_row + 1,
                               column=15).value = class_10_per
                    sheet.cell(row=current_row + 1,
                               column=16).value = class_12_per
                    sheet.cell(row=current_row + 1,
                               column=17).value = Phy_marks
                    sheet.cell(row=current_row + 1,
                               column=18).value = Chem_marks
                    sheet.cell(row=current_row + 1,
                               column=19).value = Maths_marks
    ...
    head1 = Label(win1, text="STCET", font='Georgia 60 bold',
                  relief=RAISED).grid(row=1, column=2, padx=20, pady=20)
    head2 = Label(win1, text="Admission Form", font='Granolia 23 bold',
                  relief=GROOVE).grid(row=2, column=2, padx=20, pady=20)

    fname = Label(win1, text="First Name: ", font="Arial 12 bold").grid(
        row=3, column=0, padx=20, pady=20)
    lname = Label(win1, text="Last Name: ", font="Arial 12 bold").grid(
        row=3, column=2, padx=20, pady=20)
    fname_field = Entry(win1, textvariable=f_name, width=30).grid(
        row=3, column=1, padx=20, pady=20)
    lname_field = Entry(win1, textvariable=l_name, width=30).grid(
        row=3, column=3, padx=20, pady=20)
    gender = Label(win1, text="Gender: ", font="Arial 12 bold").grid(
        row=4, column=0, padx=20, pady=20)
    x = IntVar()
    x.set(0)
    rad1 = Radiobutton(win1, text="Male", variable=x, value=1).grid(
        row=4, column=1, padx=5, pady=5)
    rad2 = Radiobutton(win1, text="Female", variable=x, value=2).grid(
        row=4, column=2, padx=5, pady=5)
    rad3 = Radiobutton(win1, text="Other", variable=x, value=3).grid(
        row=4, column=3, padx=5, pady=5)
    father = Label(win1, text="Father's Name: ", font="Arial 12 bold").grid(
        row=5, column=0, padx=20, pady=20)
    father_field = Entry(win1, textvariable=father_name, width=30).grid(
        row=5, column=1, padx=20, pady=20)
    dob = Label(win1, text="Date Of Birth (DD/MM/YYYY) :",
                font="Arial 12 bold").grid(row=5, column=2, padx=20, pady=20)
    d_field = Entry(win1, textvariable=day_data, width=10).place(x=720, y=360)
    s1 = Label(win1, text="/").place(x=790, y=360)
    m_field = Entry(win1, textvariable=month_data,
                    width=10).place(x=803, y=360)
    s2 = Label(win1, text="/").place(x=873, y=360)
    y_field = Entry(win1, textvariable=year_data, width=16).place(x=885, y=360)
    mother = Label(win1, text="Mother's Name: ", font="Arial 12 bold").grid(
        row=6, column=0, padx=20, pady=20)
    mother_field = Entry(win1, textvariable=mother_name, width=30).grid(
        row=6, column=1, padx=20, pady=20)
    contact = Label(win1, text="Contact No.: ", font="Arial 12 bold").grid(
        row=7, column=0, padx=20, pady=20)
    contact_field = Entry(win1, textvariable=phone_no_c, width=30).grid(
        row=7, column=1, padx=20, pady=20)
    ct = Label(win1, text="Father's Contact no.:", font="Arial 12 bold").grid(
        row=8, column=0, padx=20, pady=20)
    ct_field = Entry(win1, textvariable=phone_no_father, width=30).grid(
        row=8, column=1, padx=20, pady=20)
    email = Label(win1, text="Email id: ", font="Arial 12 bold").grid(
        row=9, column=0, padx=20, pady=20)
    email_field = Entry(win1, textvariable=mail_id, width=30).grid(
        row=9, column=1, padx=20, pady=20)
    ops = ['Domain_name', '@gmail.com', '@yahoo.com',
           '@hotmail.com', '@rediffmail.com']
    lmail = StringVar()
    lmail.set('Domain_name')
    e_drop = OptionMenu(win1, lmail, *ops)
    e_drop.grid(row=9, column=2)
    cadd = Label(win1, text="Current Address: ", font="Arial 12 bold").grid(
        row=6, column=2, padx=20, pady=20)
    cadd_field = Entry(win1, textvariable=current_ad, width=30).grid(
        row=6, column=3, padx=20, pady=20)
    padd = Label(win1, text="Permanent Address: ", font="Arial 12 bold").grid(
        row=7, column=2, padx=20, pady=20)
    padd_field = Entry(win1, textvariable=permanent_ad, width=30).grid(
    ...
    Button(win1, text="RESET", command=resetPage1).grid(
        row=10, column=1, padx=20, pady=20)
    Button(win1, text="CANCEL", command=close).grid(
        row=10, column=2, padx=20, pady=20)
    Button(win1, text="CONTINUE", command=cont2).grid(
        row=10, column=3, padx=20, pady=20)
...
Button(root, text="CONTINUE", command=cont1).grid(
    row=10, column=3, padx=800, pady=600)
mainloop()
